...
async function register(req, res) {
    try {
        ...
        const existingUser = await (0, db_1.default)('users').where({ phone_number }).first();
        if (existingUser) {
        ...
        const { otp, expiresAt } = (0, crypto_1.generateOTP)();
...
async function login(req, res) {
    try {
        ...
        const isMatch = await (0, crypto_1.comparePassword)(password, user.password_hash);
        if (!isMatch) {
            return res.status(401).json({
                success: false,
                message: 'Invalid phone number or password.',
