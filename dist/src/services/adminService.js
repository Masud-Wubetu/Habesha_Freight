...
exports.ADMIN_AUDIT_ACTIONS = void 0;
...
exports.changeUserRoleByAdmin = changeUserRoleByAdmin;
...
exports.listLoads = listLoads;
exports.getLoadById = getLoadById;
exports.updateLoadByAdmin = updateLoadByAdmin;
exports.listShipments = listShipments;
exports.getShipmentById = getShipmentById;
exports.updateShipmentByAdmin = updateShipmentByAdmin;
exports.listEscrow = listEscrow;
...
async function logAudit(dbClient, actorId, action, targetType, targetId, metadata = {}, ipAddress) {
...
async function countActiveAdmins(dbClient) {
...
async function getAdminDashboard(dbClient = db_1.default) {
    ...
    if (usersTable) {
        const userStats = (await dbClient('users')
            .select(dbClient.raw('COUNT(*)::int as "totalUsers"'), dbClient.raw('COUNT(*) FILTER (WHERE role = ?)::int as "totalShippers"', ['SHIPPER']), dbClient.raw('COUNT(*) FILTER (WHERE role = ?)::int as "totalDrivers"', ['DRIVER']), dbClient.raw('COUNT(*) FILTER (WHERE role = ?)::int as "totalFleetOwners"', ['FLEET_OWNER']))
            .first());
        ...
        if (hasKycStatus) {
            ...
            dashboard.pendingKyc = Number((await dbClient('users').where({ is_verified: false }).count('* as count').first())?.count ?? 0);
        }
    }
    if (vehiclesTable) {
...
async function listUsers(dbClient = db_1.default, query = {}) {
...
async function getUserById(dbClient = db_1.default, userId) {
    ...
    return {
        ...
        data: sanitizeUserRecord(user),
    };
}
async function updateUserByAdmin(dbClient = db_1.default, userId, payload, actorId) {
    if (!isValidUuid(userId)) {
        throw new Error('INVALID_ID');
    }
    ...
    return await dbClient.transaction(async (trx) => {
        ...
        if (Object.keys(allowedUpdates).length === 0) {
            return {
                ...
                data: sanitizeUserRecord(existingUser),
            };
        }
        await trx('users').where({ id: userId }).update(allowedUpdates);
        await logAudit(trx, actorId, exports.ADMIN_AUDIT_ACTIONS.USER_UPDATED, 'User', userId, {
...
async function changeUserRoleByAdmin(dbClient = db_1.default, userId, newRole, actorId) {
    if (!isValidUuid(userId)) {
        throw new Error('INVALID_ID');
    }
    ...
    return await dbClient.transaction(async (trx) => {
        ...
        await logAudit(trx, actorId, exports.ADMIN_AUDIT_ACTIONS.ROLE_CHANGED, 'User', userId, {
            previousRole,
...
async function suspendUser(dbClient = db_1.default, userId, actorId) {
    if (!isValidUuid(userId)) {
        throw new Error('INVALID_ID');
    }
    return await dbClient.transaction(async (trx) => {
        ...
        await logAudit(trx, actorId, exports.ADMIN_AUDIT_ACTIONS.USER_SUSPENDED, 'User', userId, {
...
async function activateUser(dbClient = db_1.default, userId, actorId) {
    if (!isValidUuid(userId)) {
        throw new Error('INVALID_ID');
    }
    return await dbClient.transaction(async (trx) => {
        ...
        await logAudit(trx, actorId, exports.ADMIN_AUDIT_ACTIONS.USER_ACTIVATED, 'User', userId, {
            previousState: user.status ?? user.is_verified ?? null,
        });
        const updatedUser = await trx('users').where({ id: userId }).first();
        return {
...
async function deleteUserByAdmin(dbClient = db_1.default, userId, actorId) {
    if (!isValidUuid(userId)) {
        throw new Error('INVALID_ID');
    }
    return await dbClient.transaction(async (trx) => {
        ...
        await logAudit(trx, actorId, exports.ADMIN_AUDIT_ACTIONS.USER_DELETED, 'User', userId, {
            softDelete: true,
            previousState: user.status ?? user.is_verified ?? null,
        });
        const updatedUser = await trx('users').where({ id: userId }).first();
...
async function listKycRequests(dbClient = db_1.default, query = {}) {
    ...
    let queryBuilder = dbClient('users');
    if (status)
        queryBuilder = queryBuilder.where('kyc_status', status);
    if (role)
        queryBuilder = queryBuilder.where('role', role);
    const total = Number((await queryBuilder.clone().count('* as count').first())?.count ?? 0);
...
async function getKycById(dbClient = db_1.default, userId) {
...
async function approveKyc(dbClient = db_1.default, userId, actorId, rejectionReason) {
    if (!isValidUuid(userId)) {
        throw new Error('INVALID_ID');
    }
    return await dbClient.transaction(async (trx) => {
        ...
        await logAudit(trx, actorId, exports.ADMIN_AUDIT_ACTIONS.KYC_APPROVED, 'User', userId, {
...
async function rejectKyc(dbClient = db_1.default, userId, actorId, reason) {
    if (!isValidUuid(userId)) {
        throw new Error('INVALID_ID');
    }
    ...
    return await dbClient.transaction(async (trx) => {
        ...
        await logAudit(trx, actorId, exports.ADMIN_AUDIT_ACTIONS.KYC_REJECTED, 'User', userId, {
            reason: reason.trim(),
...
async function listVehicles(dbClient = db_1.default, query = {}) {
    const page = Math.max(1, Number(query.page ?? '1'));
    const limit = Math.min(100, Math.max(1, Number(query.limit ?? '20')));
    const search = (query.search ?? '').trim();
...
async function getVehicleById(dbClient = db_1.default, vehicleId) {
...
async function verifyVehicle(dbClient = db_1.default, vehicleId, actorId) {
    if (!isValidUuid(vehicleId)) {
        throw new Error('INVALID_ID');
    }
    return await dbClient.transaction(async (trx) => {
        ...
        await logAudit(trx, actorId, exports.ADMIN_AUDIT_ACTIONS.VEHICLE_VERIFIED, 'Vehicle', vehicleId, { verificationStatus: 'VERIFIED' });
...
async function rejectVehicle(dbClient = db_1.default, vehicleId, actorId, reason) {
    if (!isValidUuid(vehicleId)) {
        throw new Error('INVALID_ID');
    }
    return await dbClient.transaction(async (trx) => {
        ...
        await logAudit(trx, actorId, exports.ADMIN_AUDIT_ACTIONS.VEHICLE_REJECTED, 'Vehicle', vehicleId, {
            reason,
...
async function listLoads(dbClient = db_1.default, query = {}) {
    ...
    if (status)
        queryBuilder = queryBuilder.where('status', status);
    if (origin)
        queryBuilder = queryBuilder.whereILike('origin_city', `%${origin}%`);
    if (destination)
        queryBuilder = queryBuilder.whereILike('destination_city', `%${destination}%`);
    if (shipper)
        queryBuilder = queryBuilder.where('shipper_id', shipper);
    const total = Number((await queryBuilder.clone().count('* as count').first())?.count ?? 0);
...
async function getLoadById(dbClient = db_1.default, loadId) {
    if (!(await tableExists(dbClient, 'loads'))) {
...
async function updateLoadByAdmin(dbClient = db_1.default, loadId, payload, actorId) {
    if (!(await tableExists(dbClient, 'loads'))) {
        throw new Error('LOADS_NOT_CONFIGURED');
    }
    if (!isValidUuid(loadId)) {
    ...
    return await dbClient.transaction(async (trx) => {
        ...
        if (Object.keys(updates).length === 0) {
            return { success: true, message: 'No eligible admin changes supplied.', data: load };
        }
        await trx('loads').where({ id: loadId }).update(updates);
        await logAudit(trx, actorId, exports.ADMIN_AUDIT_ACTIONS.LOAD_UPDATED, 'Load', loadId, {
...
async function listShipments(dbClient = db_1.default, _query = {}) {
    ...
    const total = Number((await dbClient('shipments').count('* as count').first())?.count ?? 0);
    const shipments = await dbClient('shipments').select('*').orderBy('created_at', 'desc').offset((page - 1) * limit).limit(limit);
    return {
        success: true,
        message: 'Shipments retrieved successfully.',
        data: {
            shipments: shipments.map((shipment) => shipment),
            pagination: buildPagination(page, limit, total),
        },
    };
}
async function getShipmentById(dbClient = db_1.default, shipmentId) {
    const hasShipmentsTable = await tableExists(dbClient, 'shipments');
    if (!hasShipmentsTable) {
        throw new Error('SHIPMENTS_NOT_CONFIGURED');
    }
...
async function updateShipmentByAdmin(dbClient = db_1.default, shipmentId, payload, actorId) {
    const hasShipmentsTable = await tableExists(dbClient, 'shipments');
    if (!hasShipmentsTable) {
        throw new Error('SHIPMENTS_NOT_CONFIGURED');
    }
    ...
    return await dbClient.transaction(async (trx) => {
        ...
        if (Object.keys(updates).length === 0) {
            return { success: true, message: 'No eligible shipment admin changes supplied.', data: shipment };
        }
        await trx('shipments').where({ id: shipmentId }).update(updates);
        await logAudit(trx, actorId, exports.ADMIN_AUDIT_ACTIONS.SHIPMENT_UPDATED, 'Shipment', shipmentId, {
            changedFields: Object.keys(updates),
...
async function listEscrow(dbClient = db_1.default, _query = {}) {
    ...
    const page = Math.max(1, Number(_query.page ?? '1'));
    const limit = Math.min(100, Math.max(1, Number(_query.limit ?? '20')));
    const total = Number((await dbClient('escrow_ledger').count('* as count').first())?.count ?? 0);
    ...
    return {
        ...
        data: {
            escrow: escrow.map((record) => record),
            pagination: buildPagination(page, limit, total),
        },
    };
}
async function listTransactions(dbClient = db_1.default, _query = {}) {
    ...
    const page = Math.max(1, Number(_query.page ?? '1'));
    const limit = Math.min(100, Math.max(1, Number(_query.limit ?? '20')));
    const total = Number((await dbClient('transactions').count('* as count').first())?.count ?? 0);
    const transactions = await dbClient('transactions').select('*').orderBy('created_at', 'desc').offset((page - 1) * limit).limit(limit);
    return {
        success: true,
        message: 'Transaction history retrieved successfully.',
        data: {
            transactions: transactions.map((record) => record),
            pagination: buildPagination(page, limit, total),
        },
    };
}
async function listCommissions(dbClient = db_1.default, _query = {}) {
    ...
    const page = Math.max(1, Number(_query.page ?? '1'));
    const limit = Math.min(100, Math.max(1, Number(_query.limit ?? '20')));
    const total = Number((await dbClient('commission_ledger').count('* as count').first())?.count ?? 0);
    ...
    return {
        ...
        data: {
            commissions: commissions.map((record) => record),
            pagination: buildPagination(page, limit, total),
        },
    };
}
async function listDisputes(dbClient = db_1.default, _query = {}) {
    ...
    const page = Math.max(1, Number(_query.page ?? '1'));
    const limit = Math.min(100, Math.max(1, Number(_query.limit ?? '20')));
    const total = Number((await dbClient('disputes').count('* as count').first())?.count ?? 0);
    const disputes = await dbClient('disputes').select('*').orderBy('created_at', 'desc').offset((page - 1) * limit).limit(limit);
    return {
        success: true,
        message: 'Disputes retrieved successfully.',
        data: {
            disputes: disputes.map((record) => record),
            pagination: buildPagination(page, limit, total),
        },
    };
}
async function getDisputeById(dbClient = db_1.default, disputeId) {
    const hasTable = await tableExists(dbClient, 'disputes');
    if (!hasTable) {
        throw new Error('DISPUTES_NOT_CONFIGURED');
    }
...
async function resolveDispute(dbClient = db_1.default, disputeId, actorId) {
    const hasTable = await tableExists(dbClient, 'disputes');
    if (!hasTable) {
        throw new Error('DISPUTES_NOT_CONFIGURED');
    }
    ...
    return await dbClient.transaction(async (trx) => {
        ...
        await logAudit(trx, actorId, exports.ADMIN_AUDIT_ACTIONS.DISPUTE_RESOLVED, 'Dispute', disputeId, { disputeStatus: 'RESOLVED' });
...
async function rejectDispute(dbClient = db_1.default, disputeId, actorId) {
    const hasTable = await tableExists(dbClient, 'disputes');
    if (!hasTable) {
        throw new Error('DISPUTES_NOT_CONFIGURED');
    }
    ...
    return await dbClient.transaction(async (trx) => {
        ...
        await logAudit(trx, actorId, exports.ADMIN_AUDIT_ACTIONS.DISPUTE_REJECTED, 'Dispute', disputeId, { disputeStatus: 'REJECTED' });
        const updated = await trx('disputes').where({ id: disputeId }).first();
        return {
            success: true,
...
async function listAuditLogs(dbClient = db_1.default, query = {}) {
    const page = Math.max(1, Number(query.page ?? '1'));
    const limit = Math.min(100, Math.max(1, Number(query.limit ?? '20')));
    ...
    return {
        ...
        data: {
            auditLogs: auditLogs.map((record) => record),
            pagination: buildPagination(page, limit, total),
        },
    };
}
async function getAdminAnalytics(dbClient = db_1.default, from, to) {
    ...
    const vehiclesCount = (await tableExists(dbClient, 'vehicles')) ? await dbClient('vehicles').count('* as count').first() : { count: '0' };
    const analytics = {
        usersByRole: userStatus,
        usersByStatus: [
            { status: 'ACTIVE', count: Number((await dbClient('users').where({ is_verified: true }).count('* as count').first())?.count ?? 0) },
            { status: 'INACTIVE', count: Number((await dbClient('users').where({ is_verified: false }).count('* as count').first())?.count ?? 0) },
        ],
        loadsByStatus: loadsStatus,
        totalVehicles: Number(vehiclesCount?.count ?? 0),
        totalUsers: Number((await dbClient('users').count('* as count').first())?.count ?? 0),
    };
    if (from || to) {
...
async function getSystemHealth(dbClient = db_1.default) {
